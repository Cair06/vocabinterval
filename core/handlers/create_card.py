...
async def menu_card_create_confirmation(message: types.Message, state: FSMContext):
    await message.answer("Если вы готовы начать добавление карточки нажмите 'Продолжить' ",
...
async def menu_card_create(message: types.Message, state: FSMContext):
    if message.text == 'Отмена':
        ...
        bot_message = await message.answer(
            'Создайте новую карточку!📝\n<i>Если у вас имеются несколько слов/предложений в одном из разделов '
            'разделите их запятыми</i>\n\nСлово: 🚫\nПеревод: 🚫\nТранскрипция: 🚫\nКонтекст: 🚫')
